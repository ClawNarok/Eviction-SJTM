...
public class MovePlayer : MonoBehaviour
{
    ...
    IEnumerator Dirigindo() 
    {
        ...
        {
            carro.tag = "Carro";
            MovePlayer.dirigindoCarro = false;
        }

        if (dirigindoHover)
        {
            carro.tag = "Hover";
            MovePlayer.dirigindoHover = false;
        }
    ...
    private void OnAnimatorIK(int layerIndex)
    {
        ...
        {
            anim.SetIKPosition(AvatarIKGoal.RightHand, volante.position + volante.right * .28F + volante.up * -.05f + volante.forward * -.12f);
            anim.SetIKPositionWeight(AvatarIKGoal.RightHand, 1);
            anim.SetIKPosition(AvatarIKGoal.LeftHand, volante.position - volante.right * .28F - volante.up * .05f - volante.forward * .12f);
            anim.SetIKPositionWeight(AvatarIKGoal.LeftHand, 1);
